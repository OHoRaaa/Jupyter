...
CREATE TABLE buyTbl 
(  num int AUTO_INCREMENT NOT NULL PRIMARY KEY, -- auto_increment(자동 넘버링) - primary key or unique Key
   userid  char(8) NOT NULL ,
   prodName char(6) NOT NULL,
   groupName char(4) NULL , 
   price     int  NOT NULL,
...
ALTER TABLE userTbl
	ADD CONSTRAINT PK_userTbl_userID
	PRIMARY KEY (userID);  # 참조받는 테이블의 칼럼은 primary key로 지정되어야..
...
UPDATE userTbl SET userID = 'VVK' WHERE userID='KBS';  -- error
...
UPDATE userTbl SET userID = 'VVK' WHERE userID='BBK'; -- error. 수정이 안 됨..
...
ALTER TABLE buyTbl 
	ADD CONSTRAINT FK_userTbl_buyTbl
		FOREIGN KEY (userID)
		REFERENCES userTbl (userID)
		ON UPDATE CASCADE;   # usertbl의 userid의 칼럼값을 수정하면 참조하는 테이블의 값도 동시에 수정됨...

UPDATE userTbl SET userID = 'VVK' WHERE userID='BBK';  -- error없이 수정됨..

desc usertbl;
desc buytbl;

select * from userTbl;  -- vvk로 수정됨
select * from buytbl;  -- vvk로 수정됨..
...
ALTER TABLE buyTbl 
	ADD CONSTRAINT FK_userTbl_buyTbl
		...
		REFERENCES userTbl (userID)
		ON UPDATE CASCADE
		ON DELETE CASCADE;   # 참조받는 테이블의 데이터를 삭제하면, 참조하는 테이블의 동일값의 데이터가 동시에 삭제됨..
